"""
This version is not optimized. And good example of how NOT to build a dataset
It uses a lot of memory to store sliding windows.
The new approach uses slicing (creates a view) instead of deep copy.
...
def create_dataloaders(
    ...
    train_dataset = TimeSeriesDataset(train_df, feature_cols, target_cols, config.sequence_len)
    val_dataset = (
        TimeSeriesDataset(val_df, feature_cols, target_cols, config.sequence_len) if not val_df.is_empty() else None
    )
